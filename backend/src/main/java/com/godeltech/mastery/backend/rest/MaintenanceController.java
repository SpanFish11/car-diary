...
import com.godeltech.mastery.backend.assembler.MaintenanceModelAssembler;
...
public class MaintenanceController {
  ...
  private final MaintenanceModelAssembler maintenanceModelAssembler;
  ...
  public ResponseEntity<CollectionModel<EntityModel<MaintenanceDTO>>> getAllMaintenances() {
    final List<EntityModel<MaintenanceDTO>> maintenances =
        maintenanceService.getAllMaintenance().stream()
            .map(maintenanceModelAssembler::toModel)
            .collect(Collectors.toList());
    return ok(
        CollectionModel.of(
            maintenances,
            linkTo(methodOn(MaintenanceController.class).getAllMaintenances()).withSelfRel()));
  }
}
