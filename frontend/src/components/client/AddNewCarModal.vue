  ...
  <div class="mb-4 container">
    ...
    <validation-observer ref="observer" v-slot="{ handleSubmit }">
      <b-form ref="form" @submit.stop.prevent="handleSubmit(addNewCar)">
        ...
        <validation-provider name="Equipment" :rules="{ required: true }" v-slot="validationContext">
          <b-form-group id="equipment-input-group" label="Equipment" label-for="equipment-input-group">
            <b-form-select
                id="equipment-input"
        ...
        <div v-if='$route.path === "/manager/soldcar"'>
          ...
          <validation-provider name="Ours" :rules="{ required: true }" v-slot="validationContext">
            <b-form-group v-slot="{ ariaDescribedby }">
              <b-form-radio-group
                  v-model="newCar.ours"
                  :state="getValidationState(validationContext)"
          ...
          <validation-provider name="Client" :rules="{ required: true }" v-slot="validationContext">
            <b-form-group
                label="Client"
                id="client-input-group"
                label-for="popover-input-model">
              <b-form-input placeholder="Pick a client" list="my-list-id" v-model="currentClient"
              ...
              <b-button v-b-modal.modal variant="outline-dark">Add new client</b-button>
              <b-modal id="modal" title="Add new client" hide-footer>
                <addNewClient ref="addClient" @add-client="handleClientEvent"></addNewClient>
              </b-modal>
              <b-form-invalid-feedback id="input-6">{{ validationContext.errors[0] }}</b-form-invalid-feedback>
            </b-form-group>
          </validation-provider>
        ...
        <validation-provider name="Photo"
                             ...
                             v-slot="validationContext">
          <b-form-group id="photo-input-group" label="Photo" label-for="photo-input-group">
            <b-form-file
                id="photo-input"
                name="photo-input"
                accept=".jpg, .png, .jpeg"
            ...
            ></b-form-file>
            <small class="text-muted">Selected file: {{ photoToUpload.photo ? photoToUpload.photo.name : '' }}</small>
    ...
    loadAllVins() {
      AXIOS.get('/clients').then(response => {
        ...
        response.data.forEach((client) =>
            AXIOS.get('/clients/' + client.id + '/cars').then(response => {
